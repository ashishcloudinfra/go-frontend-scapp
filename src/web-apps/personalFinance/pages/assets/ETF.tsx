...
import { addAssetItem, deleteAssetItem, fetchStockOrETFLatestData, updateAssetItem } from "../../../../app/actions/personalFinance/asset";
import { formatToIndianCurrency } from "../../../../helpers";
import { FaTrash } from "react-icons/fa6";
import { MdEditSquare } from "react-icons/md";
import { IETFNameAndCode } from "../../BootstrapPages/Asset/BootstrapETF";

export interface IETFProps {
  assetTypes: string[];
  assetItems: AssetItem[];
  etfs: IETFNameAndCode[];
  fetchAssetItems: () => void;
}

export default function ETF(props: IETFProps) {
  ...
  const assetItems = useMemo(() => {
    return props.assetItems.filter(v => v.AssetType === 'ETF');
  }, [props.assetItems])

  const etfsSelectOptions = useMemo(() => (structuredClone(props.etfs).map(v => ({ label: v.name, value: v.symbol })) as {
    label: string;
    value: string;
  }[]).sort((a, b) => {
  ...
  const etfMap = useMemo(() => structuredClone(props.etfs).reduce((memo, curr) => ({
    ...memo,
  ...
  const onFormSubmit = async (formValues: any) => {
    const stockPrice = await dispatch(fetchStockOrETFLatestData(formValues.code || ''));
    await dispatch(addAssetItem({
      ...formValues,
      name: formValues.name || etfMap[formValues.code as keyof typeof etfMap],
