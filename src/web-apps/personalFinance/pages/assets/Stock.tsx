...
import { IStockNameAndCode } from "../../BootstrapPages/Asset/BootstrapStock";

export interface IStockProps {
  assetTypes: string[];
  assetItems: AssetItem[];
  stocks: IStockNameAndCode[];
  fetchAssetItems: () => void;
}

export default function Stock(props: IStockProps) {
  ...
  const assetItems = useMemo(() => {
    return props.assetItems.filter(v => v.AssetType === 'Stock');
  }, [props.assetItems])

  const stocksSelectOptions = useMemo(() => (structuredClone(props.stocks).map(v => ({ label: v.name, value: v.symbol })) as {
    label: string;
    value: string;
  }[]).sort((a, b) => {
  ...
  const stockMap = useMemo(() => props.stocks.reduce((memo, curr) => ({
    ...memo,
  ...
  const onFormSubmit = async (formValues: any) => {
    ...
    await dispatch(addAssetItem({
      ...formValues,
      name: formValues.name || stockMap[formValues.code as keyof typeof stockMap],
